...
public class Platform : MonoBehaviour
{
    ...
    private bool[] isCoin = new bool[3] { false, false, false };
    private float width;
    ...
    private void Update()
    {
        for (int i = 0; i < obstacles.Length; i++)
        {
            ...
            else if (isTrap[i] && transform.position.x < width && !GameManager.instance.isGameover && obstacles[i].transform.localPosition.y >= 0.8) //i번째 가시가 활성화되고 발판이 플레이어에게서 멀어졌으며, 게임 오버상태가 아니며, 가시의 local y좌표가 0.8 이상일 때
            {
                ...
                if (isCoin[i] && coins[i].transform.localPosition.y >= 1.6)
                    coins[i].transform.Translate(Vector3.down * speed * Time.deltaTime);
            }
        }
    }
